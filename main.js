...
function reloadCard() {
  ...
  listCards.forEach((value, key) => {
    ...
    */
    ...
    // xoá sản phẩm khi thêm vào
    newDiv.addEventListener("click", () => {
      // Xác định sản phẩm liên quan đến biểu tượng xóa
      const deleteProductIndex = listCards.findIndex(
        (index) => index.id === value.id
      );

      if (deleteProductIndex !== -1) {
        // kiểm tra nếu sp có số lượng lớn hơn 1 thì giảm xg
        if (listCards[deleteProductIndex].quantity > 1) {
          // giam san pham
          listCards[deleteProductIndex].quantity--;

          // tính lại tiền
          listCards[deleteProductIndex].totalPrice =
            listCards[deleteProductIndex].price *
            listCards[deleteProductIndex].quantity;
        } else {
          // loại bỏ sản phẩm khỏi mảng
          listCards.splice(deleteProductIndex, 1);
        }

        // Gọi hàm reloadCard để cập nhật hiển thị giỏ hàng
        reloadCard();
      }
    });
